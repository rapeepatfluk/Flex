...
$statement = db()->prepare("SELECT j.job_id AS id,j.employer_user_id,j.job_category_id,j.work_interest_id,wi.interest_name work_interest_name,j.job_title AS title,j.job_status AS status,jc.category_slug AS job_type,j.job_description AS description,j.work_location AS location,j.work_province,j.work_schedule AS work_date,j.work_mode,j.application_deadline,j.pay_amount,j.pay_unit,j.open_positions AS positions,ep.company_name,ep.company_description,ep.company_logo_path AS company_logo,(SELECT ROUND(AVG(r.rating),1) FROM reviews r WHERE r.reviewee_user_id=j.employer_user_id AND r.review_status='visible') AS employer_rating_average,(SELECT COUNT(*) FROM reviews r WHERE r.reviewee_user_id=j.employer_user_id AND r.review_status='visible') AS employer_rating_count,(SELECT ed.document_status='approved' FROM employer_documents ed WHERE ed.employer_user_id=j.employer_user_id ORDER BY ed.submitted_at DESC,ed.employer_document_id DESC LIMIT 1) is_verified,(SELECT ji.image_file_path FROM job_images ji WHERE ji.job_id=j.job_id ORDER BY ji.display_order,ji.job_image_id LIMIT 1) AS cover_image,EXISTS(SELECT 1 FROM job_promotions jp WHERE jp.job_id=j.job_id AND jp.promotion_status='active' AND jp.starts_at<=NOW() AND jp.ends_at>NOW()) AS is_promoted FROM jobs j JOIN employer_profiles ep ON ep.user_id=j.employer_user_id JOIN job_categories jc ON jc.job_category_id=j.job_category_id LEFT JOIN work_interests wi ON wi.work_interest_id=j.work_interest_id WHERE j.job_id=? AND (?=1 OR (" . application_open_job_sql('j') . "))");
$statement->execute([(int) ($_GET['id'] ?? 0), is_role('admin') ? 1 : 0]);
$job = $statement->fetch();
if (!$job) {
    flash('error', 'ไม่พบประกาศงาน');
    redirect('jobs.php');
}
$employerReviews = review_received_list(db(), (int) $job['employer_user_id']);
                ...
                <div class="<?= is_role('worker') ? 'col-lg-8' : 'col-12' ?>">
                    <div class="job-main-content p-4 p-lg-5">
                        ...
                        <?php if ($jobRequirements['required_skills'] || $jobRequirements['preferred_skills']): ?>
                            <section class="job-section"><h2 class="h4">ความสามารถที่ต้องการ</h2><div class="d-flex flex-wrap gap-2"><?php foreach ($jobRequirements['required_skills'] as $skill): ?><span class="skill-tag required"><?= e($skill) ?> <small>จำเป็น</small></span><?php endforeach ?><?php foreach ($jobRequirements['preferred_skills'] as $skill): ?><span class="skill-tag"><?= e($skill) ?> <small>เพิ่มเติม</small></span><?php endforeach ?></div></section>
                        <?php endif ?>

                        <section class="job-section"><h2 class="h4">เกี่ยวกับผู้ว่าจ้าง</h2><p><?= e($job['company_description'] ?: 'ผู้ว่าจ้างผ่านการยืนยันตัวตนกับ FLEXJOB แล้ว') ?></p></section>
                        <section class="job-section mb-0"><h2 class="h4">ความคิดเห็นเกี่ยวกับผู้ว่าจ้าง</h2><?php $reviews = $employerReviews; $reviewTargetLabel = 'ผู้ว่าจ้าง'; require APP_ROOT . '/partials/review-list.php'; ?></section>
                    </div>
                </div>
