...
$where = [application_open_job_sql('j')];
$params = [];
if (!$showAllProvinces) {
...
$stmt = db()->prepare("SELECT j.job_id AS id,j.job_title AS title,jc.category_slug AS job_type,wi.interest_name work_interest_name,j.work_location AS location,j.work_schedule AS work_date,j.pay_amount,j.pay_unit,ep.company_name,ep.company_logo_path AS company_logo,(SELECT ROUND(AVG(r.rating),1) FROM reviews r WHERE r.reviewee_user_id=j.employer_user_id AND r.review_status='visible') employer_rating_average,(SELECT COUNT(*) FROM reviews r WHERE r.reviewee_user_id=j.employer_user_id AND r.review_status='visible') employer_rating_count,(SELECT ed.document_status='approved' FROM employer_documents ed WHERE ed.employer_user_id=j.employer_user_id ORDER BY ed.submitted_at DESC,ed.employer_document_id DESC LIMIT 1) is_verified,(SELECT ji.image_file_path FROM job_images ji WHERE ji.job_id=j.job_id ORDER BY ji.display_order, ji.job_image_id LIMIT 1) AS cover_image,promo.promotion_id FROM jobs j JOIN employer_profiles ep ON ep.user_id=j.employer_user_id JOIN job_categories jc ON jc.job_category_id=j.job_category_id LEFT JOIN work_interests wi ON wi.work_interest_id=j.work_interest_id LEFT JOIN (SELECT jp.job_id,jp.promotion_id,jp.starts_at FROM job_promotions jp WHERE jp.promotion_status='active' AND jp.starts_at<=NOW() AND jp.ends_at>NOW()) promo ON promo.job_id=j.job_id WHERE " . implode(' AND ', $where) . " ORDER BY (promo.promotion_id IS NOT NULL) DESC,promo.starts_at DESC,j.created_at DESC LIMIT {$perPage} OFFSET {$offset}");
$stmt->execute($params);
$jobs = $stmt->fetchAll();
                        ...
                        <div class="col-12 col-md-6 col-xl-4">
                            <a class="card h-100 job-card <?= $job['promotion_id'] ? 'is-promoted' : '' ?>" href="<?= BASE_URL ?>/job.php?id=<?= $job['id'] ?>">
                                ...
                                <div class="card-body d-flex flex-column p-4">
                                    <div class="d-flex flex-wrap gap-2 mb-3"><?php if ($job['promotion_id']): ?><span class="tag promoted-tag">✦ โปรโมตด้วย Pro</span><?php endif; ?><?php if ($job['work_interest_name']): ?><span class="tag"><?= e($job['work_interest_name']) ?></span><?php endif; ?><span class="tag"><?= job_type($job['job_type']) ?></span></div>
                                    <h3><?= e($job['title']) ?></h3>
