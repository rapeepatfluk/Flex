...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $pdo->prepare('INSERT INTO jobs (employer_user_id,job_category_id,work_interest_id,job_title,job_description,work_location,work_province,work_schedule,work_mode,application_deadline,pay_amount,pay_unit,open_positions) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)')->execute([
            user()['id'], $categoryId, $workInterestId, trim($_POST['title'] ?? ''), trim($_POST['description'] ?? ''),
            trim($_POST['address'] ?? ''), FLEXJOB_PROVINCE, trim($_POST['work_date'] ?? ''), $workMode,
            ...
            <section class="card border-0 shadow-sm job-post-guide mb-4">
                <div class="card-body p-4">
                    ...
                    <ol class="job-post-tips mb-0">
                        ...
                        <li><span>3</span><div><strong>เลือกทักษะอย่างตรงไปตรงมา</strong><small>เลือกเฉพาะทักษะที่จำเป็นต่อการทำงาน</small></div></li>
                    </ol>
                </div>
            </section>
            ...
            <form method="post" enctype="multipart/form-data" class="job-post-form">
                ...
                <section class="card border-0 shadow-sm job-post-section mb-4"><div class="card-body p-4 p-lg-5">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6"><label class="form-label" for="work_date">วัน / ช่วงเวลาทำงาน</label><input id="work_date" class="form-control" name="work_date" value="<?= e($formData['work_date'] ?? '') ?>" maxlength="255" placeholder="เช่น 23–24 ส.ค. 09:00–18:00"></div>
                ...
                <section class="card border-0 shadow-sm job-post-section mb-4"><div class="card-body p-4 p-lg-5">
                    <div class="job-post-section-heading"><span class="job-post-step">03</span><div><h2 class="h4 mb-1">ทักษะและรูปประกอบ</h2><p class="mb-0 text-secondary">เลือกเฉพาะทักษะที่ผู้สมัครควรมีเพื่อให้ระบบจับคู่ได้แม่นยำ</p></div></div>
