...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $pdo->prepare("INSERT INTO jobs (employer_user_id,job_category_id,job_title,job_description,work_location,work_province,work_schedule,work_mode,application_deadline,pay_amount,pay_unit,open_positions) SELECT ?,job_category_id,?,?,?,?,?,?,?,?,?,? FROM job_categories WHERE category_slug=?")
            ->execute([
                user()['id'],
                trim($_POST['title'] ?? ''),
