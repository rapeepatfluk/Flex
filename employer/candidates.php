...
$jobStatement = $pdo->prepare("SELECT job_id,job_title FROM jobs WHERE employer_user_id=? AND job_status='published' AND (application_deadline IS NULL OR application_deadline>=CURDATE()) ORDER BY created_at DESC");
...
$province = mb_strtolower(trim($_GET['province'] ?? ''), 'UTF-8');
$minimumScore = max(0, min(100, (int) ($_GET['min_score'] ?? 0)));
$workers = array_values(array_filter($workers, function (array $worker) use ($query, $province, $minimumScore): bool {
    $searchable = mb_strtolower(implode(' ', [$worker['name'], $worker['headline'], $worker['skill_names']]), 'UTF-8');
    if ($query !== '' && !str_contains($searchable, $query)) return false;
    if ($province !== '' && !str_contains(mb_strtolower((string) $worker['work_province'], 'UTF-8'), $province)) return false;
...
<main class="container py-5">
    ...
    <?php else: ?>
        <form class="card border-0 shadow-sm mb-4" method="get"><div class="card-body"><div class="row g-3">
            <div class="col-lg-4"><label class="form-label" for="job">จับคู่กับประกาศ</label><select class="form-select" id="job" name="job" onchange="this.form.submit()"><?php foreach ($employerJobs as $job): ?><option value="<?= $job['job_id'] ?>" <?= $selectedJobId === (int) $job['job_id'] ? 'selected' : '' ?>><?= e($job['job_title']) ?></option><?php endforeach ?></select></div>
            <div class="col-lg-3"><label class="form-label" for="q">ชื่อ Headline หรือทักษะ</label><input class="form-control" id="q" name="q" value="<?= e($_GET['q'] ?? '') ?>" placeholder="เช่น Excel, Canva"></div>
            <div class="col-lg-3"><label class="form-label" for="province">จังหวัด</label><input class="form-control" id="province" name="province" value="<?= e($_GET['province'] ?? '') ?>"></div>
        ...
        <div class="row g-4">
            <?php foreach ($workers as $worker): ?><div class="col-12 col-lg-6"><article class="card h-100 border-0 shadow-sm candidate-card"><div class="card-body p-4">
                <div class="d-flex justify-content-between align-items-start gap-3"><div><h3 class="h5 mb-1"><?= e($worker['name']) ?></h3><p class="text-secondary mb-2"><?= e($worker['headline'] ?: 'ยังไม่ได้ระบุ Headline') ?></p></div><?php if ($worker['match']['score'] !== null): ?><span class="match-score"><?= $worker['match']['score'] ?>%</span><?php else: ?><span class="badge text-bg-light border">ข้อมูลไม่พอ</span><?php endif ?></div>
