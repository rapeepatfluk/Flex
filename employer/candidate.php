...
$jobStmt = $pdo->prepare("SELECT j.job_title FROM jobs j WHERE j.job_id=? AND j.employer_user_id=? AND j.work_province=? AND " . application_open_job_sql('j'));
...
$workerRatingSummary = review_received_summary($pdo, $workerId);
$workerReviews = review_received_list($pdo, $workerId);
...
<main class="container py-5"><a class="link-primary d-inline-block mb-4" href="<?= BASE_URL ?>/employer/candidates.php?job=<?= $jobId ?>">← กลับไปผลการค้นหา</a>
    <div class="row g-4"><div class="col-lg-8"><section class="card border-0 shadow-sm"><div class="card-body p-4 p-md-5">
        ...
        <div class="mt-3"><?php $ratingSummary = $workerRatingSummary; require APP_ROOT . '/partials/rating-summary.php'; ?></div>
        ...
        <section class="border rounded-4 p-3 p-md-4 mt-4"><p class="eyebrow mb-1">WORKER REVIEWS</p><h2 class="h5 mb-3">ความคิดเห็นจากงานที่ผ่านมา</h2><?php $reviews = $workerReviews; $reviewTargetLabel = 'ผู้หางาน'; require APP_ROOT . '/partials/review-list.php'; ?></section>
