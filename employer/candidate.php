...
<main class="container py-5"><a class="link-primary d-inline-block mb-4" href="<?= BASE_URL ?>/employer/candidates.php?job=<?= $jobId ?>">← กลับไปผลการค้นหา</a>
    <div class="row g-4"><div class="col-lg-8"><section class="card border-0 shadow-sm"><div class="card-body p-4 p-md-5">
        <div class="d-flex justify-content-between gap-3"><div><p class="eyebrow">CANDIDATE PROFILE</p><h1 class="h2 mb-1"><?= e($candidate['name']) ?></h1><p class="text-secondary"><?= e($candidate['headline'] ?: 'ยังไม่ได้ระบุ Headline') ?></p></div><?php if ($candidate['match']['score'] !== null): ?><span class="match-score large"><?= $candidate['match']['score'] ?>%</span><?php endif ?></div>
